...
void type_resolution(void **dest, void *src) {
    //printf("ADDRS: %p %p\n", *(void**)dest, src);
...
int main()
{
    ...
    s.data = (void*)&ex;
    Custom *retr;
    type_resolution((void*)&retr, s.data);
    printf("%d %d\n", retr->a, retr->b);
    return 0;
}
...
int a;
func((DSData)&a);
correct value is now in a, that is usable as a normal int variable
type resolution is iternal, no need of manual casting in your code
*/
